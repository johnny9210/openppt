...
4. 라이트 테마 — 밝은 배경, 깔끔한 카드, 기업 발표 수준 퀄리티
...
```jsx
<div style={{
  background: THEME.background,  // #F5F7FA — 밝은 회색 배경
  height: "100%", padding: "48px 60px",
  position: "relative", overflow: "hidden",
}}>
  {/* 선택: 우측 상단 미세한 기하학 장식 (연한 선 + 점) */}
</div>
```
...
<div style={{
  ...
  borderRadius: 16,
  padding: "24px 28px",
  boxShadow: THEME.cardShadow,  // 0 2px 8px rgba(0,0,0,0.06)
...
### 타이포그래피
- 슬라이드 제목: fontSize 32~40, fontWeight 800, color THEME.text (#1A202C)
- 제목 아래 컬러 악센트 바: width 48, height 4, borderRadius 2, background THEME.primary
- 섹션 제목: fontSize 20~24, fontWeight 700, color THEME.text
- 카드 제목: fontSize 17~19, fontWeight 600, color THEME.text
- 본문/설명: fontSize 14~15, color THEME.textSecondary (#64748B), lineHeight 1.6
- 대형 지표: fontSize 48~64, fontWeight 800, color THEME.primary

### 색상 규칙
- 배경: THEME.background (#F5F7FA) — 밝은 회색
- 카드: THEME.card (#FFFFFF) — 순백
- 제목: THEME.text (#1A202C) — 진한 다크
- 본문: THEME.textSecondary (#64748B) — 회색
- 포인트: THEME.primary 또는 THEME.accent
- 상태: THEME.green(긍정), THEME.red(부정), THEME.yellow(주의)
- 아이콘 배지 배경: THEME.iconBg1, THEME.iconBg2 (교차)

### 장식 요소 (선택적)
- 우측 상단: 미세한 기하학 네트워크 패턴 (연한 회색, opacity 0.15)
- 좌상단 코너: 작은 톱니바퀴/원형 장식 (연한 회색, opacity 0.1)
- 이런 장식은 SVG path로 구현 가능

## 절대 금지
- 컴포넌트 이름은 반드시 [컴포넌트 이름]에 지정된 이름 사용
- import 문 작성 금지 (이미 상위에서 import됨)
- const THEME = ... 재선언 금지 (이미 전역)
- 다크 테마/글래스모피즘 스타일 금지 (rgba 반투명 배경 금지)
- 설명, 마크다운 없이 순수 React 컴포넌트 코드만 출력
- JSX 텍스트 안에서 < > 문자 직접 사용 금지 -> {"<"} 또는 &lt; &gt; 사용
- boxShadow는 반드시 THEME.cardShadow를 직접 사용. 템플릿 리터럴이나 삼항 연산자로 boxShadow를 구성하지 말 것
- 템플릿 리터럴 안 삼항 연산자는 반드시 완전한 형태로: ${조건 ? 값A : 값B} (: 생략 금지)

## 사용 가능
- THEME.primary, THEME.accent, THEME.background, THEME.text, THEME.textSecondary
- THEME.card, THEME.cardBorder, THEME.cardShadow, THEME.iconBg1, THEME.iconBg2
- THEME.red, THEME.yellow, THEME.green, THEME.subtleBg, THEME.divider
...
def _extract_code(text: str) -> str:
    """Extract code from LLM response, removing markdown fences and fixing common errors."""
