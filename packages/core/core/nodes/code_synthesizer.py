"""
Phase 3: Code Synthesizer (3-Pass Architecture)
Pass 1 (Vision): Design image -> React + Tailwind layout code (structure only, no text)
Pass 2 (Code):   Layout code + text content -> Final React component with text
Pass 3 (Code):   Fit to 16:9 container and adjust Tailwind classes
...
All styling uses Tailwind CSS utility classes (not inline styles).
"""
...
LAYOUT_SYSTEM_PROMPT = """당신은 디자인 이미지를 React + Tailwind CSS 코드로 변환하는 전문 개발자입니다.

★ 핵심 임무: 디자인 이미지의 시각적 구조를 React + Tailwind CSS 클래스로 정확히 재현합니다.
...
<styling_rules>
## ★ Tailwind CSS 사용 규칙
- 모든 스타일링은 className에 Tailwind 유틸리티 클래스를 사용하세요
- style={{}} 인라인 스타일은 절대 사용하지 마세요 (recharts fill/stroke만 예외)

## 커스텀 테마 클래스 (tailwind.config에 정의됨)
### 색상
- bg-primary, text-primary, border-primary: 주 브랜드 색상
- bg-accent, text-accent, border-accent: 보조 색상
- bg-primary/10, bg-accent/10: 투명도 변형 (Tailwind 내장 문법)
- text-heading: 제목 텍스트 색상
- text-body: 본문/설명 텍스트 색상 (#64748B)
- bg-card: 카드 배경 (흰색)
- border-card-border: 카드 테두리
- bg-slide-bg: 슬라이드 배경
- text-danger/bg-danger, text-warning/bg-warning, text-success/bg-success
...
### 자주 쓰는 Tailwind 패턴
- 카드: bg-card rounded-2xl shadow-card border border-card-border p-6
- 아이콘 배지: w-14 h-14 rounded-full bg-primary flex items-center justify-center text-white text-2xl
- 액센트 바: w-12 h-1 rounded-sm bg-primary mx-auto
- 호버 효과: hover:shadow-card-hover hover:-translate-y-0.5 transition-all
- 그라디언트: bg-gradient-to-br from-primary to-accent
</styling_rules>

<task>
## 재현할 요소
1. **레이아웃 구조**: flex, grid, gap-*, p-*, m-* — 이미지와 최대한 동일하게
2. **카드 스타일**: bg-card rounded-2xl shadow-card border border-card-border
3. **아이콘 배지**: rounded-full, w-14 h-14, bg-primary/bg-accent, 이모지 "●"
4. **장식 요소**: 액센트 바, 배경 패턴, 그라디언트
5. **호버/트랜지션**: hover:shadow-card-hover, transition-all 등 인터랙션
6. **색상**: 커스텀 테마 클래스 사용 (bg-primary, text-heading 등)
...
const KeyPointsSlide = ({ content }) => {
  ...
  return (
    <div className="h-full bg-slide-bg px-16 py-12">
      {/* 제목 영역 */}
      <div className="text-center mb-2" />
      <div className="w-12 h-1 rounded-sm bg-primary mx-auto mb-8" />
      <div className="grid grid-cols-2 gap-5 max-w-[800px] mx-auto">
        {points.map((_, i) => (
          <div key={i} className="bg-card rounded-2xl p-6 shadow-card border border-card-border flex items-start gap-4 hover:shadow-card-hover hover:-translate-y-0.5 transition-all">
            <div className={`w-14 h-14 rounded-full shrink-0 flex items-center justify-center text-2xl text-white ${i % 2 === 0 ? "bg-primary" : "bg-accent"}`}>
              {"●"}
            </div>
...
- SLIDE_IMAGES 참조 금지 — 이미지를 배경으로 사용하지 않음
- className="h-full ..." 최상위 컨테이너 필수
- ★ style={{}} 인라인 스타일 절대 사용 금지 — Tailwind 클래스만 사용 (recharts fill/stroke 예외)
...
<theme_for_recharts>
THEME 객체는 recharts 컴포넌트의 fill, stroke 등에만 사용하세요:
THEME.primary, THEME.accent, THEME.text, THEME.textSecondary
...
TEXT_INSERT_SYSTEM_PROMPT = """당신은 React + Tailwind CSS 슬라이드 컴포넌트에 텍스트 콘텐츠를 삽입하는 전문 개발자입니다.

★ 핵심 임무: 주어진 레이아웃 코드의 빈 영역에 텍스트 콘텐츠를 삽입하고, Tailwind 클래스로 스타일을 미세 조정합니다.
...
<task>
## 해야 할 것
1. **레이아웃 유지**: 기존 코드의 Tailwind 클래스, 카드 배치, 그리드, 색상을 그대로 유지
...
## 텍스트 Tailwind 스타일 가이드
- 메인 제목: text-3xl font-extrabold text-heading
- 부제목/설명: text-sm text-body leading-relaxed
- 카드 제목: text-base font-semibold text-heading
- 카드 설명: text-xs text-body leading-relaxed
- 강조 수치/메트릭: text-xl font-bold text-primary
- 라벨/태그: text-xs font-medium
- 라벨 배지: text-xs font-medium bg-primary/10 text-primary px-2 py-0.5 rounded-full
</task>
...
- 기존 Tailwind 클래스 대폭 변경 (색상, 그림자, border-radius 등)
- ★ style={{}} 인라인 스타일 추가 (recharts fill/stroke 예외)
- 컴포넌트 이름 변경
- import 문 추가
- const THEME 재선언
</do_not>
...
- const THEME = ... 재선언 금지
- className="h-full ..." 유지
- ★ Tailwind 클래스만 사용 (style={{}} 금지, recharts fill/stroke 예외)
...
<available_libraries>
- recharts: BarChart, Bar, Cell, LineChart, Line, PieChart, Pie, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer 등 (fill/stroke에 THEME 사용)
...
{"이 이미지의 시각적 구조(카드 배치, 아이콘, 색상, 간격, 장식)를 React + Tailwind CSS 클래스로 정확히 재현하세요." if has_image else f"'{slide_type}' 타입에 적합한 깔끔하고 전문적인 레이아웃을 Tailwind CSS 클래스로 설계하세요."}
...
레이아웃 구조(Tailwind 클래스, 카드 배치, 색상)는 그대로 유지하고 텍스트와 이모지만 추가하세요."""
...
FITTING_SYSTEM_PROMPT = """당신은 React + Tailwind CSS 슬라이드 컴포넌트의 레이아웃을 16:9 컨테이너에 맞추는 전문가입니다.

★ 핵심 임무: 주어진 코드가 16:9 비율(약 960×540px) 컨테이너 안에 모든 요소가 들어가도록 Tailwind 클래스를 조정합니다.
...
<analysis>
## 분석 절차
1. 최상위 컨테이너의 padding 클래스(px-*, py-*) 확인 → 내부 영역 계산
2. 제목+설명 영역의 텍스트 크기(text-*) 및 여백(mb-*) 추정
3. 본문 콘텐츠 영역 크기 추정:
   - grid/flex 레이아웃 행 수 × (카드 크기 + gap-*)
   - 카드 내부 padding(p-*) + 아이콘 크기(w-*, h-*) + 텍스트 줄 수
4. 전체 높이 합산 → 540px(16:9 기준) 초과 여부 판단
</analysis>

<adjustments>
## 넘칠 때 조정 우선순위 (Tailwind 클래스 변경)
1. **padding 축소**: px-16 py-12 → px-10 py-8 또는 px-8 py-6
2. **gap 축소**: gap-5 → gap-3 또는 gap-2
3. **카드 내부 padding 축소**: p-6 → p-4 또는 p-3
4. **fontSize 축소**: text-3xl → text-2xl, text-base → text-sm, text-sm → text-xs
5. **아이콘 배지 축소**: w-14 h-14 → w-10 h-10 또는 w-9 h-9
6. **margin 축소**: mb-8 → mb-4 또는 mb-3
7. **그리드 재배치**: grid-cols-2 → grid-cols-3 (항목 5~6개인 경우)
</adjustments>

<rules>
## 규칙
- 디자인의 시각적 정체성(색상 클래스, 카드 스타일, 구조)은 유지
- 콘텐츠를 삭제하거나 숨기지 마세요 — Tailwind 클래스만 조정
- 이미 16:9에 충분히 들어가는 코드는 수정하지 말고 그대로 출력
- 컴포넌트 이름, import, THEME 재선언 금지 등 기존 제약 유지
- ★ style={{}} 사용 금지 — Tailwind 클래스만 (recharts fill/stroke 예외)
</rules>"""
...
넘칠 가능성이 있으면 Tailwind 클래스(padding, gap, text-size 등)를 축소하여 맞추세요.
